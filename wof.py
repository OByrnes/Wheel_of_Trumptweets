...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        ...
        return poss_guess
    def getMove(self,category,obscuredPhrase, guessed):
        poss_letters=self.getPossibleLetters(guessed)
        if len(poss_letters) ==0:
            return "pass"
        howGood=self.smartCoinFlip()
        if howGood:
            for i in range(len(self.SORTED_FREQUENCIES)):
                if self.SORTED_FREQUENCIES[-(i+1)] in poss_letters:
                    return self.SORTED_FREQUENCIES[-(i+1)]
        else:
            return random.choice(poss_letters)
